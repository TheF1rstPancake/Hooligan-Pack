...
function New-Changelog {
  if ($ENABLE_CHANGELOG_GENERATOR_MODULE `
    ...
    Write-Host
    Write-Host "$InstanceRoot/$LAST_MODPACK_ZIP_NAME.zip and $InstanceRoot/$CLIENT_ZIP_NAME.zip"
    Write-Host 
...
function Update-Modlist {
  if ($ENABLE_MODLIST_CREATOR_MODULE) {
    ...
    java -jar $ModlistCreatorJar --html --detailed --output ".\" --manifest "$InstanceRoot\$CLIENT_ZIP_NAME.zip" 
